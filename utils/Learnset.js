...
class Learnset {
  ...
  constructor (pokemon, dex) {
    if (typeof pokemon === 'string') {
      pokemon = dex.species.get(pokemon);
    }

    pokemon.learnset = dex.species.getLearnset(pokemon.id);
    ...
    do {
      ...
      if (!pokemon.learnset) {
        if (pokemon.baseSpecies !== pokemon.name) {
          pokemon = dex.species.get(pokemon.baseSpecies);
          srcMon = 'base species';
          continue;
        }
        break;
      ...
      if (pokemon.name === 'Lycanroc-Dusk') {
        pokemon = dex.species.get('Rockruff-Dusk');
      } else if (pokemon.prevo) {
        pokemon = dex.species.get(pokemon.prevo);
        pokemon.learnset = dex.species.getLearnset(pokemon.id);
        srcMon = 'prevo';
