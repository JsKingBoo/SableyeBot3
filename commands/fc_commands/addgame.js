...
module.exports = {
  ...
  process: async function (msg, flags, author, fcm) {
    ...
    return (success ? `Registered ${user.games[user.games.length - 1].game} for @${author.tag} as ${user.games[user.games.length - 1].name}.` : `Error: could not add game information.`);
  }
};
